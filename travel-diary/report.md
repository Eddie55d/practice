# Кейс-задача: Разработка Web-версии “Дневник путешествий”
...
```
### Заметка
Никакого настоящего API key Yandex Map не запрашивалось. В связи с этим можно наблюдать на в кладке Console браузера ошибку типа 
(Yandex Maps JS API): Invalid API key. Для демонстрации это не критично, иногда может не подгрузится карта, не удивляйтесь!
